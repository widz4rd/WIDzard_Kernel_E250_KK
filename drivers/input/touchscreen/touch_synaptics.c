	...
	if (likely(ts->ts_data.interrupt_status_reg & INTERRUPT_MASK_ABS0)) {
		...
		while (touch_finger_bit_mask) {
			...
			data->curr_data[finger_index].width_major = 
				TS_SNTS_GET_WIDTH_MAJOR(ts->ts_data.finger.finger_reg[finger_index][REG_WX], 
									    ts->ts_data.finger.finger_reg[finger_index][REG_WY]);
			data->curr_data[finger_index].width_minor = 
				TS_SNTS_GET_WIDTH_MINOR(ts->ts_data.finger.finger_reg[finger_index][REG_WX], 
									    ts->ts_data.finger.finger_reg[finger_index][REG_WY]);
			data->curr_data[finger_index].width_orientation = 
				TS_SNTS_GET_ORIENTATION(ts->ts_data.finger.finger_reg[finger_index][REG_WY],
									    ts->ts_data.finger.finger_reg[finger_index][REG_WX]);
			data->curr_data[finger_index].pressure = 
				TS_SNTS_GET_PRESSURE(ts->ts_data.finger.finger_reg[finger_index][REG_Z]);

			data->curr_data[finger_index].status = FINGER_PRESSED;
#ifdef CUST_G2_TOUCH
			z_sum += data->curr_data[finger_index].pressure;
			if (data->curr_data[finger_index].y_position == 0 && 
				data->curr_data[finger_index].width_orientation == 0 && 
					data->curr_data[finger_index].width_major > 10)
				data->curr_data[finger_index].y_position = 3;

			if (data->curr_data[finger_index].y_position == 1920 && 
				data->curr_data[finger_index].width_orientation == 0 && 
					data->curr_data[finger_index].width_major > 10)
